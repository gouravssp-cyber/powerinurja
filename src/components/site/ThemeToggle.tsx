  ...
  return (
    <button
      ...
      className={`relative inline-flex h-8 w-14 items-center rounded-full border transition-colors duration-200 ${
        theme === "dark" ? "border-accent bg-accent/20" : "border-border bg-muted"
      }`}
    >
      <span className="sr-only">Toggle color mode</span>

      <span
        className={`absolute left-1 top-1 h-6 w-6 rounded-full border border-border bg-background shadow-sm transition-transform duration-200 ${
